...
theorem qubitXZ_eq_phase_qubitPauli (x z : ZMod 2) :
  ...
  rcases cases2 x with rfl | rfl <;> rcases cases2 z with rfl | rfl <;>
    ...
    norm_num [qubitXZ, qubitPauli, qubitX, qubitY, qubitZ, negOnePow,
      iPow, Matrix.one_apply, Complex.I_sq]
  all_goals norm_num [Matrix.one_apply,
    show (2 : ZMod 2) = 0 from rfl,
    show (1 : ZMod 2).val = 1 from rfl,
    show (-(ZMod.cast (1 : ZMod 2) : ZMod 4)).val = 3 from rfl,
    Complex.I_sq, pow_succ]
  all_goals decide
