...
/-!
# The Pauli coefficient vector, and `‖O‖_2̄ = ‖x‖_{ℓ²}`
...
normalized Schatten 2-norm `‖O‖_2̄` (the *Pauli 2-norm*) equals the `ℓ²` norm of the coefficient
...
* `PauliString.highSet`, `PauliString.highNorm`: the region `{p : w < |p|}` and the high-weight
  norm `‖O_{≥ w+1}‖_2̄` (`apd:eq:def_high_weight_norm`).

## Main results

* `PauliString.pauliNormSq_eq_trace`: `‖O‖_2̄² = 2^{-n} Tr(O† O)`.
* `PauliString.sum_norm_coeff_sq`, `PauliString.norm_coeffVec`: Parseval, `‖x‖_{ℓ²} = ‖O‖_2̄`.
...
`sum_norm_coeff_sq` is the identity `‖x‖_{ℓ²}² = ‖O‖_2̄²`, and it is proved by a **direct character
...
reproducing property `coeff_truncOp`: an operator whose coefficients all vanish has `‖·‖_2̄ = 0`
...
/-- The squared **Pauli 2-norm** `‖O‖_2̄² = 2^{-n} Tr(O† O)`, written entrywise as
...
/-- The **Pauli 2-norm** `‖O‖_2̄`, the normalized Schatten 2-norm: `‖O‖_2̄² = 2^{-n} Tr(O† O)`,
...
/-- `‖O‖_2̄² = 2^{-n} Tr(O† O)`: the entrywise definition `pauliNormSq` agrees with the trace
...
/-- **Parseval: `‖O‖_2̄ = ‖x‖_{ℓ²}`, squared.** This is the identity the proof of
...
/-- **`‖O‖_2̄ = ‖x‖_{ℓ²}`**, as norms rather than squares. -/
...
/-- **`‖O‖_2̄ = ‖x‖_{ℓ²}`**, as an identity of norms: the Pauli 2-norm of the operator *is* the
...
/-- **The high-weight norm** `‖O_{≥ w+1}‖_2̄` of `apd:eq:def_high_weight_norm`,
...
/-- The high-weight mass never exceeds the total mass: `‖O_{≥ w+1}‖_2̄ ≤ ‖O‖_2̄` at every
