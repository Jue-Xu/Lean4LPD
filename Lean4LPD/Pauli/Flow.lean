...
1. **The coefficient vector** — `Pauli/Coeff.lean`, with `‖O‖_2̄ = ‖x‖_{ℓ²}`.
...
* **Rung `0` is the reservoir, not `rungWeight ko kh 0`.** `Ladder/Defs.lean` explains why:
  ...
  `w_0 = k_o - (k_h-1)`. So `ladderN` sets `N 0 g := ‖O^{(g)}‖_2̄`, which satisfies `reservoir`
...
/-- The total mass is conserved along the trajectory: `‖O^{(g)}‖_2̄ = ‖O‖_2̄` for every `g`. -/
...
/-- **The ladder family** `N m g := ‖(U_g† O U_g)_{≥ w_m + 1}‖_2̄`
(`apd:eq:def_high_weight_norm`), with rung `0` the reservoir `‖O^{(g)}‖_2̄`. -/
